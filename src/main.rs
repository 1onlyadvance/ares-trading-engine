...
async fn main() -> Result<()> {
    ...
    tracing_subscriber::registry()
        .with(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "ares_trading_engine=info".into()),
        )
        .with(tracing_subscriber::fmt::layer())
        .init();

    info!("Starting ARES Trading Engine");
    info!("===================================");
    ...
    info!("ARES Trading Engine is running. Press Ctrl+C to stop.");
    ...
    info!("ARES Trading Engine stopped.");
    Ok(())
}
